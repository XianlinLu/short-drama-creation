#!/usr/bin/env python3
"""Validate lumina-novel-generator structure and sample evidence."""
...
def validate_skill_md() -> None:
    ...
    frontmatter = parts[1]
    if "name: lumina-novel-generator" not in frontmatter:
...
def validate_manifest() -> None:
    ...
    if data.get("name") != "lumina-novel-generator":
...
def validate_lumina_runtime() -> None:
    runtime = read("LUMINA_SYSTEM_INSTRUCTIONS.md")
    for term in [
        ...
        if term not in runtime:
            fail(f"LUMINA_SYSTEM_INSTRUCTIONS.md missing required term: {term}")

    skill_text = read("SKILL.md")
    for term in [
        "Lumina Canvas Runtime",
        "LUMINA_SYSTEM_INSTRUCTIONS.md",
        "only components connected",
    ]:
        if term not in skill_text:
            fail(f"SKILL.md missing Lumina runtime term: {term}")
...
def main() -> None:
    ...
    validate_lumina_runtime()
    print("[OK] lumina-novel-generator structure, runtime prompt, and samples validated")
